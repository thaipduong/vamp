...
namespace vamp::planning
{
    ...
    template <typename Robot, std::size_t rake, std::size_t resolution>
    inline static auto reduce_flatpath_vertices(
        Path<Robot::dimension> &path,
        const collision::Environment<FloatVector<rake>> &environment,
        const ReduceSettings &settings) -> bool
    {
        ...
        for (auto i = 0U, no_change = 0U; i < max_steps or no_change < max_empty_steps; ++i, ++no_change)
        {
            ...
            if (validate_poly_motion<Robot, rake, resolution>(path[point_0], path[point_1], environment))
            {
    ...
    template <typename Robot, std::size_t rake, std::size_t resolution>
    inline static auto shortcut_flatpath(
        Path<Robot::dimension> &path,
        const collision::Environment<FloatVector<rake>> &environment,
        const ShortcutSettings & /*settings*/) -> bool
    {
        ...
        for (auto i = 0U; i < path.size() - 2; ++i)
        {
            ...
            {
                if (validate_poly_motion<Robot, rake, resolution>(path[i], path[j], environment))
                {
    ...
    template <typename Robot, std::size_t rake, std::size_t resolution>
    inline auto flat_simplify(
        const Path<Robot::flatstate_dimension> &path,
        const collision::Environment<FloatVector<rake>> &environment,
        const SimplifyFlatSettings &settings) -> PlanningResult<Robot::flatstate_dimension>
    {
        auto start_time = std::chrono::steady_clock::now();

        PlanningResult<Robot::flatstate_dimension> result;
        ...
        { return reduce_flatpath_vertices<Robot, rake, resolution>(result.path, environment, settings.reduce); };
        const auto shortcut = [&result, &environment, settings]()
        { return shortcut_flatpath<Robot, rake, resolution>(result.path, environment, settings.shortcut); };
        ...
        if (path.size() == 2 or
            validate_poly_motion<Robot, rake, resolution>(path.front(), path.back(), environment))
        {
